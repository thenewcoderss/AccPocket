...
financeRouter.get("/transaction-titles", asyncRoute(async (req, res) => {
  const rows = await prisma.transactionTitle.findMany({ where: { userId: req.userId! }, select: { id: true, name: true, _count: { select: { transactions: true } } }, orderBy: { name: "asc" } });
  res.json({ success: true, data: rows.map(({ _count, ...row }) => ({ ...row, canDelete: _count.transactions === 0, transactionCount: _count.transactions })) });
}));
financeRouter.post("/transaction-titles", asyncRoute(async (req, res) => {
  ...
  const row = await prisma.transactionTitle.create({ data: { userId: req.userId!, name } });
  res.status(201).json({ success: true, data: { ...row, canDelete: true, transactionCount: 0 } });
}));
financeRouter.delete("/transaction-titles/:id", asyncRoute(async (req, res) => {
  const title = await prisma.transactionTitle.findFirst({ where: { id: String(req.params.id), userId: req.userId! }, include: { _count: { select: { transactions: true } } } });
  if (!title) throw new AppError(404, "TRANSACTION_TITLE_NOT_FOUND", "Transaction Title was not found");
  if (title._count.transactions) throw new AppError(409, "TRANSACTION_TITLE_IN_USE", "This Transaction Title is used by transaction history and cannot be deleted");
  await prisma.transactionTitle.delete({ where: { id: title.id } });
  res.json({ success: true, data: null });
}));

async function assertTransactionTitle(userId: string, titleId: string, tx: Prisma.TransactionClient) {
  const title = await tx.transactionTitle.findFirst({ where: { id: titleId, userId } });
...
financeRouter.get("/transactions", asyncRoute(async (req, res) => {
  ...
  res.json({ success: true, data: { items: rows.map(row => ({ id: row.id, type: row.type, amount: row.entries.find(e => e.amount.isPositive())?.amount.abs().toString() ?? row.entries[0]?.amount.abs().toString(), date: row.date.toISOString(), title: row.title?.name ?? (row.type === "TRANSFER" ? "Internal transfer" : row.description || "Untitled transaction"), description: row.description, category: row.category, accountName: row.entries.find(e => e.amount.isNegative())?.account.name ?? row.entries[0]?.account.name, destinationAccountName: row.type === "TRANSFER" ? row.entries.find(e => e.amount.isPositive())?.account.name : undefined })), page: query.page, total } });
}));
financeRouter.post("/transactions", asyncRoute(async (req, res) => {
  ...
  const created = await prisma.$transaction(async tx => {
    ...
    await assertTransactionTitle(req.userId!, input.titleId, tx);
    ...
    const transaction = await tx.transaction.create({ data: { userId: req.userId!, categoryId: input.categoryId, titleId: input.titleId, type: input.type, date: input.date, description: input.description, notes: input.notes, entries: { create: { accountId: account.id, amount: signed, resultingBalance: next } } } });
