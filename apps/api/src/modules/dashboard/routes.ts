...
dashboardRouter.get("/", asyncRoute(async (req, res) => {
  ...
  const [report, accounts, budgets, goals, recent] = await Promise.all([
    reportData(req.userId!, period), prisma.account.findMany({ where: { userId: req.userId!, archived: false }, select: { id: true, name: true, type: true, openingBalance: true, currentBalance: true, currency: true, archived: true }, orderBy: { createdAt: "asc" } }),
