...
dashboardRouter.get("/", asyncRoute(async (req, res) => {
  ...
  const [report, accounts, budgets, goals, recent] = await Promise.all([
    ...
    prisma.transaction.findMany({ where: { userId: req.userId!, deletedAt: null }, select: { id: true, type: true, date: true, description: true, category: true, entries: { select: { amount: true, account: { select: { name: true } } } } }, orderBy: [{ date: "desc" }, { createdAt: "desc" }], take: 6 })
  ]);
