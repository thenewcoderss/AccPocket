...
export async function reportData(userId: string, period: "day" | "week" | "month", from?: Date, to?: Date, detailRowLimit = env.REPORT_DETAIL_ROW_LIMIT) {
  ...
  const transactions = rows.slice(0, detailRowLimit).map(row => {
    ...
    return { id: row.id, date: row.date.toISOString(), type: row.type, title: row.title?.name ?? (row.type === "TRANSFER" ? "Internal transfer" : row.description || "Untitled transaction"), description: row.description, category: row.category?.name ?? "—", account, amount: amount.toString() };
  });
  ...
  const detailedAmounts = rows.map((row, index) => { const source = row.entries.find(entry => entry.amount.isNegative())?.account.name; const destination = row.entries.find(entry => entry.amount.isPositive())?.account.name; return { id: row.id, date: row.date.toISOString(), type: row.type, title: row.title?.name ?? (row.type === "TRANSFER" ? "Internal transfer" : row.description || "Untitled transaction"), description: row.description, account: row.type === "TRANSFER" ? `${source ?? "Wallet"} → ${destination ?? "Wallet"}` : row.entries[0]?.account.name ?? "—", amount: transactionAmounts[index].amount }; });
