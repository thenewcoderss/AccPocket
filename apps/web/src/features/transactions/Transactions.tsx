...
export function Transactions() {
  ...
  const transactions = useQuery({ queryKey: ["transactions"], queryFn: () => api.get<Result>("/transactions") });
  ...
  const add = useMutation({ mutationFn: (body: unknown) => mode === "TRANSFER" ? api.post("/transfers", body) : api.post("/transactions", body), onSuccess: () => { void queryClient.invalidateQueries(); setMode(null); } });
  ...
  function openTransfer() {
    ...
    setAccountNotice(null); setSourceAccountId(""); add.reset(); setMode("TRANSFER");
  }
  ...
  function submit(event: FormEvent<HTMLFormElement>) {
    ...
    add.mutate(mode === "TRANSFER"
      ? { sourceAccountId: data.accountId, destinationAccountId: data.destinationAccountId, amount: data.amount, description: data.description, date: data.date }
  ...
  return <Page title="Transactions" description="Income, expenses, and transfers in date order." action={<><button className="btn-secondary !px-3 sm:!px-4" disabled={accounts.isLoading || accounts.isError} aria-label="Create internal transfer" onClick={openTransfer}><ArrowRightLeft size={18}/><span className="hidden min-[420px]:inline">Transfer</span></button><button className="btn-primary !px-3 sm:!px-4" disabled={accounts.isLoading || accounts.isError} onClick={openEntry}><Plus size={18}/>Add</button></>}>
    ...
    {mode && <Modal title={mode === "TRANSFER" ? "Internal transfer" : `Add ${mode.toLowerCase()}`} close={closeModal}><form className="space-y-5" onSubmit={submit}>
      ...
      <label className="block"><span className="label">{mode === "TRANSFER" ? "From account" : "Account"}</span><select className="input" name="accountId" required value={mode === "TRANSFER" ? sourceAccountId : undefined} onChange={mode === "TRANSFER" ? event => setSourceAccountId(event.target.value) : undefined}><option value="">Choose account</option>{availableAccounts.map(account => <option key={account.id} value={account.id}>{account.name}</option>)}</select></label>
      {mode === "TRANSFER" && <label className="block"><span className="label">To account</span><select className="input" name="destinationAccountId" required><option value="">Choose account</option>{availableAccounts.filter(account => account.id !== sourceAccountId).map(account => <option key={account.id} value={account.id}>{account.name}</option>)}</select><span className="mt-1.5 block text-xs text-slate-500">Transfers move records between your accounts. They are not income or expenses.</span></label>}
      {mode !== "TRANSFER" && <label className="block"><span className="label">Category</span><select className="input" name="categoryId"><option value="">Uncategorized</option>{categories.data?.filter(category => category.type === mode).map(category => <option key={category.id} value={category.id}>{category.name}</option>)}</select>{categories.error && <span className="mt-1.5 block text-xs text-amber-700">Categories could not be loaded. You can save this entry as uncategorized.</span>}</label>}
      <label className="block"><span className="label">Amount</span><input className="input-amount" name="amount" type="number" inputMode="decimal" min="0.01" max="999999999999999.99" step="0.01" placeholder="0.00" required/></label>
      ...
      <label className="block"><span className="label">Description</span><input className="input" name="description" maxLength={120} defaultValue={mode === "TRANSFER" ? "Internal transfer" : ""} required/></label>
      {mode !== "TRANSFER" && <label className="block"><span className="label">Notes <span className="font-normal text-slate-400">(optional)</span></span><textarea className="input min-h-24 py-3" name="notes" maxLength={500}/></label>}
