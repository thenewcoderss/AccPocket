...
export function Dashboard() {
  ...
  return <Page title={`Hello, ${user?.name.split(" ")[0]}`} description="Here’s a clear view of your money this month.">
    ...
    <div className="mt-8 grid gap-6 lg:grid-cols-[minmax(0,.9fr)_minmax(0,1.1fr)]">
      ...
      <section><h2 className="section-title">Recent activity</h2><p className="mb-3 mt-1 text-sm text-slate-500">Your latest recorded entries</p>{d.recentTransactions.length ? <div className="space-y-2">{d.recentTransactions.map(t => <div className="card flex min-w-0 items-center justify-between gap-3" key={t.id}><div className="min-w-0"><p className="truncate font-semibold">{t.description}</p><p className="mt-1 truncate text-xs text-slate-500">{new Date(t.date).toLocaleDateString()} · {t.accountName}</p></div><div className="shrink-0 text-right"><span className="block text-[10px] font-bold uppercase tracking-wide text-slate-400">{t.type === "EXPENSE" ? "Expense" : "Income"}</span><Money value={t.amount} currency={user?.defaultCurrency} className={t.type === "EXPENSE" ? "font-bold text-red-600" : "font-bold text-green-700"}/></div></div>)}</div> : <Empty title="No activity yet" text="Add an account, then record your first income or expense."/>}</section>
    </div>
