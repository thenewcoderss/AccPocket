...
export function Planning() {
  ...
  function openGoal() {
    ...
    if (!availableAccounts.length) { setAccountNotice("Create a wallet first before adding a savings or emergency fund goal."); return; }
  ...
  function openContribution(goal: Goal) {
    if (!accounts.isSuccess) return;
    if (!contributionAccounts(accounts.data, goal.destinationAccountId).length) { setAccountNotice("Create another wallet first before adding a contribution."); return; }
    setAccountNotice(null); save.reset();
  ...
  const accountAction = <Link className="btn-tonal" to="/accounts">Go to wallet</Link>;
  return <Page title="Plans" description="Set simple limits and make steady progress." action={<div className="flex gap-2"><button className="btn-secondary" disabled={categories.isLoading || categories.isError || budgets.isLoading || budgets.isError} onClick={openBudget}>Budget</button><button className="btn-primary" disabled={accounts.isLoading || accounts.isError} onClick={openGoal}><Plus size={18}/>Goal</button></div>}>
    ...
    {!goals.isLoading && !goals.error && !goals.data?.length && <Empty title="No goals yet" text={accounts.isSuccess && !availableAccounts.length ? "Create a wallet first before adding a savings or emergency fund goal." : "Create a savings or emergency fund target."} action={accounts.isSuccess && !availableAccounts.length ? <Link className="btn-primary" to="/accounts">Create wallet</Link> : undefined}/>}
